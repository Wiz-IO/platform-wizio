...
uint8_t getBlockAt(const uint8_t level[], uint8_t x, uint8_t y)
{
  ...
  {
    DEBUG("ERROR getBlockAt\n");
    return E_FLOOR;
  }

  // y is read in inverse order
  return pgm_read_byte(level + (((LEVEL_HEIGHT - 1 - y) * LEVEL_WIDTH + x) / 2)) >> (!(x % 2) * 4) // displace part of wanted bits
